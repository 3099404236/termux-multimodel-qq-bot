...
class Main(star.Star):
    def __init__(self, context: star.Context) -> None:
        ...
        self.active_reply_c = ActiveReplyCommands(self.context)
        ...
        self.stack_c = StackControlCommands(self.context)
    ...
    @filter.permission_type(filter.PermissionType.ADMIN)
    @filter.command("ar")
    async def active_reply_short(
        self, event: AstrMessageEvent, action: GreedyStr = ""
    ) -> None:
        """查看或切换群聊主动回复。/ar 状态|开启|关闭"""
        await self.active_reply_c.active_reply(event, action)

    @filter.permission_type(filter.PermissionType.ADMIN)
    @filter.command("activereply")
    async def active_reply(
        self, event: AstrMessageEvent, action: GreedyStr = ""
    ) -> None:
        """查看或切换群聊主动回复。/activereply 状态|开启|关闭"""
        await self.active_reply_c.active_reply(event, action)

    @filter.permission_type(filter.PermissionType.ADMIN)
    @filter.command("主动回复")
    async def active_reply_cn(
        self, event: AstrMessageEvent, action: GreedyStr = ""
    ) -> None:
        """查看或切换群聊主动回复。/主动回复 状态|开启|关闭"""
        await self.active_reply_c.active_reply(event, action)

    @filter.permission_type(filter.PermissionType.ADMIN)
    @filter.command("stack")
    async def stack(self, event: AstrMessageEvent, action: GreedyStr = "") -> None:
        """控制服务栈。/stack 状态|关闭|重启|启动"""
        await self.stack_c.stack(event, action)
