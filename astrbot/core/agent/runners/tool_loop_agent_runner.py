...
from astrbot.core.runtime_error_visibility import (
    log_suppressed_runtime_error,
    runtime_error_message_for_user,
)
...
class ToolLoopAgentRunner(BaseAgentRunner[TContext]):
    @staticmethod
    def _should_suppress_tool_progress_message(text: str) -> bool:
        """Hide tool preambles like '正在查资料' when a tool call will follow."""
        ...
        patterns = (
            r"^(正在)?(查|查询|搜|搜索|检索|阅读|看|找)(一下|下|相关)?(资料|信息|内容)?$",
            r"^(我|我先|我去|我来|让我|先)(先)?(查|搜|搜索|检索|阅读|看|找)(一下|下|相关)?(资料|信息|内容)?$",
            r"^(稍等|等等)(我)?(先)?(查|搜|搜索|检索|阅读|看|找)(一下|下|相关)?(资料|信息|内容)?$",
            r"^(letme|ill|iwill)?(check|search|lookup|lookitup|readup)(this|that|it|foryou|first)?$",
            r"^(searching|checking|lookingitup|readingup)(this|that|it)?$",
        )
        ...
        has_lookup_verb = any(
            token in normalized
            for token in ("查", "查询", "搜", "搜索", "检索", "阅读", "看", "找")
        )
        has_progress_marker = any(
            token in normalized
            for token in ("正在", "稍等", "等等", "过一会", "一会", "一会儿", "稍后")
        )
        has_info_target = any(
            token in normalized
            for token in ("资料", "信息", "内容", "文章", "论文", "来源")
        )
        if has_lookup_verb and (has_progress_marker or has_info_target):
            if not any(
                token in normalized
                for token in ("答案", "结论", "是", "叫", "如下", "如下所示", "文章是")
            ):
                return True
        return False
    ...
    async def step(self):
        ...
        if (
            ...
            and self._should_suppress_tool_progress_message(llm_resp.completion_text)
        ):
            logger.info(
                "Suppressing tool progress preamble before tool call: %s",
        ...
        if llm_resp.role == "err":
            ...
            visible_error_text = runtime_error_message_for_user(event, error_text)
            if visible_error_text is None:
                log_suppressed_runtime_error(
                    event,
                    source="tool_loop_agent_runner.step",
            ...
            yield AgentResponse(
                type="err",
                data=AgentResponseData(
                    chain=MessageChain().message(visible_error_text),
                ),
            )
            return
        ...
        # Only expose assistant text after the tool loop is done.
        # Tool-calling turns are agent-internal reasoning/state, not final user replies.
        if not llm_resp.tools_call_name:
