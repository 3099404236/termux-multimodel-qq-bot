...
async def run_agent(
    ...
    while step_idx < max_step + 1:
        ...
        try:
            ...
            if visible_error_text is not None:
                if agent_runner.streaming:
                    yield MessageChain().message(visible_error_text)
                else:
                    astr_event.set_result(
                        MessageEventResult().message(visible_error_text)
                    )
            else:
                log_suppressed_runtime_error(
                    astr_event,
                    source="astr_agent_run_util.run_agent",
                    error_text=err_msg,
                )
            return
