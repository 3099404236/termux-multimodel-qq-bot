...
from astrbot.core.runtime_error_visibility import (
    ...
    should_surface_runtime_error_to_user,
)
...
async def run_third_party_agent(
    ...
    try:
        ...
        if should_surface_runtime_error_to_user(event):
            yield MessageChain().message(err_msg), True
        else:
            log_suppressed_runtime_error(
                event,
                source="third_party_agent_sub_stage.run_third_party_agent",
                error_text=err_msg,
            )
            yield MessageChain(), True
    ...
    def finalize(
        ...
        *,
        ...
        if not final_resp or not final_resp.result_chain:
            if suppress_error_output and self.has_error:
                return [], True
        ...
        if suppress_error_output and is_runner_error:
            return [], True
...
class ThirdPartyAgentSubStage(Stage):
    ...
    async def _handle_streaming_response(
        ...
        *,
        ...
        if runner.done():
            suppress_error_output = not should_surface_runtime_error_to_user(event)
