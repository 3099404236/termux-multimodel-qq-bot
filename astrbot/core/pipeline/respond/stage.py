...
class RespondStage(Stage):
    ...
    async def process(
        ...
        if result.result_content_type == ResultContentType.AGENT_RUNNER_ERROR:
            raw_error_text = MessageChain(result.chain).get_plain_text(
                with_other_comps_mark=True
            )
            visible_error_text = runtime_error_message_for_user(
                event,
                raw_error_text,
            )
        else:
            raw_error_text = ""
            visible_error_text = None

        if (
            result.result_content_type == ResultContentType.AGENT_RUNNER_ERROR
            and visible_error_text is None
        ):
            log_suppressed_runtime_error(
                event,
                source="respond_stage.process",
