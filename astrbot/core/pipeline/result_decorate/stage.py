...
# QQ rejects a forward node whose content is too large and fails the whole send
# with "转发消息失败，生成节点为空". Observed here: 19584 bytes delivered fine, 30981
# bytes did not.
FORWARD_NODE_MAX_BYTES = 20000


def split_text_by_bytes(text: str, limit: int) -> list[str]:
    """Split text into pieces of at most `limit` UTF-8 bytes.

    Breaks are preferred at a newline, then at a space, so a piece rarely ends
    mid-sentence. Characters are never split.
    """
    ...
    while start < length:
        ...
        if end == start:  # a single character larger than the limit
            end = start + 1

        # Look for a break in the last fifth so pieces stay near the limit
        # instead of collapsing into tiny fragments.
        window = start + max(1, (end - start) * 4 // 5)
        cut = text.rfind("\n", window, end)
        if cut == -1:
            cut = text.rfind(" ", window, end)
        cut = end if cut <= start else cut + 1
...
def _build_forward_nodes(
    ...
    *,
    uin: str,
    limit: int = FORWARD_NODE_MAX_BYTES,
) -> list:
    """Pack a message chain into forward nodes that each stay under `limit`."""
    nodes: list[Node] = []
    current: list = []
    current_bytes = 0
    ...
    def flush() -> None:
        ...
        if current:
            nodes.append(Node(uin=uin, name="AstrBot", content=current))
            current = []
            current_bytes = 0
    ...
    for comp in chain:
        if not isinstance(comp, Plain):
            current.append(comp)
            continue
        for piece in split_text_by_bytes(comp.text, limit):
            piece_bytes = len(piece.encode("utf-8"))
            if current_bytes and current_bytes + piece_bytes > limit:
                flush()
            current.append(Plain(piece))
            current_bytes += piece_bytes
    flush()

    if not nodes:
        return [Node(uin=uin, name="AstrBot", content=[*chain])]
    if len(nodes) == 1:
        # Unchanged from before for the common case: one node, sent as-is.
        return nodes
    # Several nodes must travel inside one Nodes container, otherwise the
    # aiocqhttp adapter sends each of them as its own forward message.
    return [Nodes(nodes)]
        ...
        if len(result.chain) > 0:
            ...
            if (
                ...
                if word_cnt > self.forward_threshold:
                    result.chain = _build_forward_nodes(
                        result.chain,
