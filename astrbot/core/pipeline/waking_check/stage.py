...
class WakingCheckStage(Stage):
    ...
    def _extract_plain_text_mention_aliases_from_prompt(
    ...
    async def _get_plain_text_mention_aliases(
        ...
        try:
            ...
            if persona_manager is not None:
                ...
                system_prompt = (persona or {}).get("prompt", "")
                aliases.update(
                    self._extract_plain_text_mention_aliases_from_prompt(system_prompt)
                )
        except Exception as exc:
            logger.debug(f"Failed to resolve plain-text mention aliases: {exc!s}")

        return sorted(aliases, key=len, reverse=True)

    async def _match_plain_text_bot_mention(
        self, event: AstrMessageEvent
    ) -> tuple[bool, str]:
        if event.is_private_chat():
            return False, event.message_str

        text = event.message_str.strip()
        if not text.startswith(PLAIN_TEXT_MENTION_PREFIXES):
            return False, event.message_str

        aliases = await self._get_plain_text_mention_aliases(event)
        for alias in aliases:
            for prefix in PLAIN_TEXT_MENTION_PREFIXES:
                mention_prefix = f"{prefix}{alias}"
    ...
    async def process(
        ...
        if not is_wake:
            (
                matched_plain_text_mention,
                stripped_message,
            ) = await self._match_plain_text_bot_mention(event)
            if matched_plain_text_mention:
                is_wake = True
                event.is_wake = True
                event.is_at_or_wake_command = True
                event.message_str = stripped_message
