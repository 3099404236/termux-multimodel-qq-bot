...
class MockToolProgressProvider(MockProvider):
    async def text_chat(self, **kwargs) -> LLMResponse:
        self.call_count += 1
        if self.call_count == 1:
            return LLMResponse(
                role="assistant",
                completion_text="正在查询相关资料，可能要过一会儿才能展示好",
                tools_call_name=["test_tool"],
                tools_call_args=[{"query": "test"}],
                tools_call_ids=["call_123"],
                usage=TokenUsage(input_other=10, output=5),
...
@pytest.mark.asyncio
async def test_tool_progress_preamble_is_not_emitted_as_llm_result(
    runner, provider_request, mock_tool_executor, mock_hooks
):
    provider = MockToolProgressProvider()
    ...
    llm_texts = [
        response.data["chain"].get_plain_text(with_other_comps_mark=True)
        for response in responses
        if response.type == "llm_result"
    ]
    assert len(llm_texts) == 1
    assert "正在查询相关资料，可能要过一会儿才能展示好" not in llm_texts
    assert llm_texts[-1] == "这是我的最终回答"

    assistant_messages = [
        msg for msg in runner.run_context.messages if msg.role == "assistant"
    ]
    persisted_text = "\n".join(str(msg.content) for msg in assistant_messages)
    assert "正在查询相关资料" not in persisted_text
